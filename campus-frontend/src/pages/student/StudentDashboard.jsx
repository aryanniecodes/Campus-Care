...
const StatCard = ({ label, value, color = "text-gray-900", icon, delay = 0 }) => (
  <motion.div 
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.4, delay }}
    whileHover={{ scale: 1.02, y: -4 }}
    className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between transition-all duration-200"
  >
    <div>
      ...
      <div className="space-y-8">
        ...
        >
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 font-medium">Here's an overview of your campus maintenance requests.</p>
          </div>
          <motion.button 
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate("/student/create")}
            className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-blue-500/30 transition-all flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <StatCard label="In Progress" value={stats.pending} color="text-yellow-600" icon={icons.pending} delay={0.2} />
          <StatCard label="Resolved" value={stats.completed} color="text-green-600" icon={icons.completed} delay={0.3} />
        </div>
        ...
        >
          ...
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map(i => (
                <div key={i} className="animate-pulse flex space-x-4 p-4 rounded-xl border border-gray-50">
            ...
            <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
              <p className="text-sm font-medium text-gray-500">You have no recent requests.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {complaints.slice(0, 5).map((c, i) => (
                <motion.div 
                  ...
                  key={c._id} 
                  className="flex items-center justify-between p-4 rounded-xl hover:bg-slate-50 transition-colors duration-200 border border-transparent hover:border-slate-100 group cursor-pointer"
                >
                  <div className="flex-1 min-w-0 pr-4">
                    <p className="text-sm font-bold text-gray-900 truncate group-hover:text-blue-600 transition-colors">{c.title}</p>
